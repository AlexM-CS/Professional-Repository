...
# This is Version 1.1, a low-level bot that makes suggestion based entirely on letter frequency
...
# UPDATES FROM 1.0:
# (1) New function "noWordsWith()": removes all words containing the letter given.
# The reason this was not implemented in the previous version is that it caused bugs when duplicate letters existed.
# With some new functionality added to remove this possibility, it will not longer cause the program to occasionally fail.
# (2) New function "checkForDuplicates()": returns a letter if there are multiple of that letter in a word.

# Various UI Updates:
# (1) When prompted "Your guess will be: " an answer is required to be given
# (2) Better readability implemented
# (3) Instructions are now only read on the first guess
# (4) New function "failed()": The program now stops automatically if over six attempts are made
...
def greeting(): # Greets the user
    print()
    print("## Hello user. This is Wordle Solver version 1.1. Let's get solving! ##")
