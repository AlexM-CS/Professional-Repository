# Alexander Myska          2-20-2024
# Wordle-bot is a Wordle-solving algorithm I have created entirely on my own
# This is Version 3.1, a version of the bot that (in theory) functions for the currently live version of Wordle.
# The way it does this is by having the ability to utilize any database for words, as long
# as that database can be provided in a .txt file by the user.

# UPDATES FROM 3.0:
# (1) Added a default option for the database select. When "Default" is typed,
# the bot will use the scrabble dictionary list for its algorithm.
# (2) New function "getOtherList1()", "getOtherList2()", "getOtherList3()": gets the list not being primarily used
# Now, a multiplier is given to words that appear in other word lists as well, giving prevalence
# to more commonly-used words
# (3) The functions multiplyBonus(), findBestWord(), and getDatabase() needed to be changed slightly
# to support the new functionality described above
# (4) New function "noWordsWithout()": edits the list further to ensure that a word contains the given letter
...
def getOtherList1(): # Creates an alternate list to compare against
...
def getOtherList2(): # Creates an alternate list to compare against
...
def getOtherList3(): # Creates an alternate list to compare against
    wordFile = open("Wordle_Bot_Folder/Wordle_Resources/scrabbleDictionary.txt","r")
...
def greeting(): # Greets the user
    print()
    print("## Hello user. This is Wordle Solver version 3.1. Let's get solving! ##")
...
def findBestWord(wordList, otherList1, otherList2, otherList3): # Calculates a "score" for every word in the provided list and then selects the word with the highest score
...
def main(): # Function that runs the bot
    ...
    otherList1 = getOtherList1()
    otherList2 = getOtherList2()
    otherList3 = getOtherList3()
    ...
    while (running):
        ...
        bestWord = findBestWord(wordList, otherList1, otherList2, otherList3)
